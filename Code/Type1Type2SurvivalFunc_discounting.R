# Helper functions for section 3.3
...
ObjHelper <- function(n, vintermed, vfinal, zmin, zmax, tt, p) {
  if (tt == "A") {
    c((p$cw/p$gamma) * integrate(function(xx) {
        ...
        dnorm(xx) * (1-exp(-1*p$gamma*w)) #exp(-p$gamma*w) is the discount factor for value xx
      }, zmin, zmax)$value,
      (p$cp*p$lE/p$gamma) * integrate(function(xx) {
        w <- sapply(xx, function(x) tA(muT=x*sqrt(vintermed-vintermed^2/p$s0T^2), v=vfinal, n=n, p))
        dnorm(xx) * (1-exp(-1*p$gamma*min(w, n/p$lE)))
      }, zmin, zmax)$value)
  } else if (tt == "B") {
    #print("cost wait")
